...
{
    public class Cheb1SpectralOdeOperator : ISpectralOdeOperator<double[][]>
    {
        ...
        public Cheb1SpectralOdeOperator(IEnumerable<Func<double, double>> phi, IEnumerable<Func<double, double>> phiSobolev, double[] quadratureNodes)
        {
            _phi = phi;
        ...
        public void SetParams(DynFunc<double>[] odeRightSides, double[] initialValues, int partialSumOrder)
        {
            ...
            _weight = (x => 1.0 / Math.Sqrt(1.0 - x * x));
        }
        ...
        private double CalcCoeff(int i, int k, double[][] fArgs)
        {
            ...
            var qk = _nodes.Select((t, j) => _f[i].Invoke(fArgs[j]) * _phiCached[k](t) * _weight(t));
